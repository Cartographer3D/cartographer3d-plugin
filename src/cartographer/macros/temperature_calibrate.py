...
class Chamber(Protocol):
    ...
    def turn_on_cooling(self, cooler: str) -> None: ...
    def turn_off_cooling(self, cooler: str) -> None: ...
    ...
    def _fit_temperature_model(self, data_per_height: dict[float, list[Sample]]) -> CoilCalibrationConfiguration:
        ...
        for height, samples in data_per_height.items():
            try:
                ...
                logger.warning("Failed to process data for height %.1f: %s", height, e)
                continue

        if len(a_values) < 2:
            msg = "Insufficient valid datasets for temperature model fitting"
            raise RuntimeError(msg)

        # Calculate reference frequency and temperature
        fmin = float(np.mean(freq_values))

        # Fit linear relationships for parameters
...
def process_data(samples: list[Sample]) -> tuple[float, float, float]:
    ...
    if len(samples) < 10000:
        msg = f"Insufficient samples for calibration: {len(samples)} (need at least 10000)"
        raise RuntimeError(msg)

    dv = int(len(samples) / 10000)
    if dv > 1:
        samples = samples[::dv]
    samples = samples[200:]
