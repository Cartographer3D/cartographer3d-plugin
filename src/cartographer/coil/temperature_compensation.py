...
if TYPE_CHECKING:
    ...
    from cartographer.interfaces.printer import CoilCalibrationReference, Mcu
...
class CoilTemperatureCompensationModel(TemperatureCompensationModel):
    ...
    def __init__(self, config: CoilCalibrationConfiguration, mcu: Mcu):
        self._mcu = mcu
        self.a_a = config.a_a
        self.a_b = config.a_b
        self.b_a = config.b_a
    ...
    def param_linear(self, x: float, a: float, b: float) -> float:
        return a * x + b
    ...
    def _interpolate_parameters(self, freq_offset: float) -> tuple[float, float]:
        ...
        param_a_interp = self.param_linear(freq_offset, self.a_a, self.a_b)
        param_b_interp = self.param_linear(freq_offset, self.b_a, self.b_b)
        return param_a_interp, param_b_interp

    def _build_quadratic_coefficients(self, temp_source: float, freq_offset: float) -> tuple[float, float, float]:
        """Build coefficients for the quadratic equation ax² + bx + c = 0."""
        temp_sq = temp_source**2
        temp = temp_source
